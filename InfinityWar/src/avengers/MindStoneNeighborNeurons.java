...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        int synSize = StdIn.readInt();
        String[][] synapse = new String[synSize][2];
        ...
        for (int i = 0; i < synapse.length; i ++){
            for (int j = 0; j < array.length; j++){
                ...
                if (synapse[i][0].equals(array[j]) == false){
                    keepsteak = array[j];
                }
            }
        }
        ...
        for (int i = 0; i < synapse.length; i++){
            if (synapse[i][1].equals(keepsteak)){
                StdOut.println(synapse[i][0]);
            }
        }
    }
}
