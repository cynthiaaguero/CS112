...
public class PredictThanosSnap {
    ...
    private static void dfs (int node, boolean[] vux, int[][] matrix) {

         
        vux[node] = true;

        for (int i = 0; i < vux.length; i++){
            if((!vux[i]) && matrix[node][i] == 1){
                dfs(i, vux, matrix);
            }
        }
    }
    ...
    public static void main (String[] args) {
        ...
        boolean dux[] = new boolean[numberNodes]; //collects deleted indexes 
        boolean vux[] = new boolean[numberNodes]; //collects visited indexes 

        StdRandom.setSeed(seed);

        int[][] matrix = new int[numberNodes][numberNodes];
